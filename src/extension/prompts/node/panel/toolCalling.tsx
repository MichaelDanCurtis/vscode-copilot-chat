...
export class ChatToolCalls extends PromptElement<ChatToolCallsProps, void> {
	...
	override async render(
		...
		if (
			!this.props.promptContext.tools ||
			!this.props.toolCallRounds?.length
		) {
			return;
		}

		// Create a child instantiation service with IBuildPromptContext registered
		const hydratedInstantiationService =
		...
		const toolCallRounds = this.props.toolCallRounds.flatMap((round, i) => {
			return this.renderOneToolCallRound(
				round,
				i,
				this.props.toolCallRounds!.length,
				hydratedInstantiationService,
				token,
			);
		});
	 ...
	 */
	private renderOneToolCallRound(
		round: IToolCallRound,
		index: number,
		total: number,
		hydratedInstantiationService: IInstantiationService,
		token?: CancellationToken,
	): PromptElement[] {
		let fixedNameToolCalls = round.toolCalls.map((tc) => ({
			...tc,
			name: this.toolsService.validateToolName(tc.name) ?? tc.name,
		}));
		if (this.props.isHistorical) {
			fixedNameToolCalls = fixedNameToolCalls.filter(
				(tc) => tc.id && this.props.toolCallResults?.[tc.id],
			);
		}
		...
		const assistantToolCalls: Required<ToolCall>[] = fixedNameToolCalls.map(
			(tc) => ({
				type: 'function',
		...
		const statefulMarker = round.statefulMarker && (
		...
		const thinking = !this.props.isHistorical && round.thinking && (
			<ThinkingDataContainer thinking={round.thinking} />
		);
		const phase =
			round.phase && round.phaseModelId === this.promptEndpoint.model ? (
				<PhaseDataContainer phase={round.phase} />
			) : undefined;
		const compaction = round.compaction && (
			<CompactionDataContainer compaction={round.compaction} />
		);
		children.push(
		...
		for (const [i, toolCall] of fixedNameToolCalls.entries()) {
			...
			children.push(
				<KeepWith
					priority={index}
					flexGrow={index + 1}
					flexReserve={`/${1 / reserve1N}`}
				>
					{hydratedInstantiationService.invokeFunction(
						buildToolResultElement,
						{
							toolCall: toolCall,
							toolInvocationToken:
								this.props.promptContext.tools!
									.toolInvocationToken,
							toolCallResult:
								this.props.toolCallResults?.[toolCall.id!],
							allowInvokingTool: !this.props.isHistorical,
							validateInput:
								round.toolInputRetry <
								MAX_INPUT_VALIDATION_RETRIES,
							requestId: this.props.promptContext.requestId,
							toolCallMode:
								this.props.toolCallMode ??
								CopilotToolMode.PartialContext,
							isLast:
								!this.props.isHistorical &&
								i === fixedNameToolCalls.length - 1 &&
								index === total - 1,
							enableCacheBreakpoints:
								this.props.enableCacheBreakpoints ?? false,
							truncateAt: this.props.truncateAt,
							sessionId:
								this.props.promptContext.request?.sessionId,
 ...
 */
function buildToolResultElement(
	...
	const promptContext: IBuildPromptContext =
		accessor.get(IBuildPromptContext);
	...
	async function getToolResult(sizing: PromptSizing) {
		...
		if (!props.toolCallResult && !props.allowInvokingTool) {
			throw new Error(
				`Missing tool call result for "${props.toolCall.id}" (${props.toolCall.name})`,
			);
		}
		...
		const copilotTool = toolsService.getCopilotTool(
			props.toolCall.name as ToolName,
		);
		if (toolResult === undefined) {
			let inputObj: unknown;
			let validation: ToolValidationOutcome =
				ToolValidationOutcome.Unknown;
			if (props.validateInput) {
				...
				if ('error' in validationResult) {
					...
					extraMetadata.push(
						new ToolFailureEncountered(props.toolCall.id),
					);
					toolResult = textToolResult(
						validationResult.error + toolErrorSuffix,
					);
				} else {
			...
			let outcome: ToolInvocationOutcome =
				toolResult === undefined
					...
					: ToolInvocationOutcome.InvalidInput;
			if (toolResult === undefined) {
				try {
					if (
						promptContext.tools &&
						!promptContext.tools.availableTools.find(
							(t) => t.name === props.toolCall.name,
						)
					) {
						outcome = ToolInvocationOutcome.DisabledByUser;
						throw new Error(
							`Tool ${props.toolCall.name} is currently disabled by the user, and cannot be called.`,
						);
					}

					if (copilotTool?.resolveInput) {
						inputObj = await copilotTool.resolveInput(
							inputObj,
							promptContext,
							props.toolCallMode,
						);
					...
					const hookResult =
						await chatHookService.executePreToolUseHook(
							props.toolCall.name,
							inputObj,
							props.toolCall.id,
					...
					const subAgentInvocationId =
						promptContext.request?.subAgentInvocationId;
					...
					const parentTraceContext =
						otelService.getActiveTraceContext();
					const invocationOptions: LanguageModelToolInvocationOptions<unknown> =
						{
							...
							chatStreamToolCallId:
								props.toolCall.id.split('__vscode')[0],
							preToolUseResult: hookResult
								? {
										permissionDecision:
											hookResult.permissionDecision,
										permissionDecisionReason:
											hookResult.permissionDecisionReason,
					...
					if (transcriptSessionId) {
						...
						try {
							parsedArgs = JSON.parse(props.toolCall.arguments);
						} catch {
							parsedArgs = props.toolCall.arguments;
						}
						sessionTranscriptService.logToolExecutionStart(
							transcriptSessionId,
							props.toolCall.id,
							props.toolCall.name,
							parsedArgs,
					...
					toolResult = await toolsService.invokeToolWithEndpoint(
						props.toolCall.name,
						invocationOptions,
						promptEndpoint,
						props.token,
					...
					sendInvokedToolTelemetry(
						promptEndpoint.acquireTokenizer(),
						telemetryService,
						props.toolCall.name,
						toolResult,
					);
					...
					if (props.toolCall.name === 'render_a2ui') {
						...
						try {
							...
							a2uiSurfaceId = (
								(parsed as Record<string, unknown>)['doc'] as
					...
					if (transcriptSessionId) {
						sessionTranscriptService.logToolExecutionComplete(
							transcriptSessionId,
							props.toolCall.id,
							true,
						);
					...
					if (errResult.isCancelled) {
						...
						outcome =
							outcome === ToolInvocationOutcome.DisabledByUser
								? outcome
								: ToolInvocationOutcome.Error;
						extraMetadata.push(
							new ToolFailureEncountered(props.toolCall.id),
						);
						logService.error(
							`Error from tool ${props.toolCall.name} with args ${props.toolCall.arguments}`,
	...
	if (
		...
		const promise = getToolResult({
			tokenBudget: 1,
			countTokens: () => 1,
			endpoint: { modelMaxPromptTokens: 1 },
		});
		call = () => promise;
	} else {
		call = getToolResult;
...
async function sendToolCallTelemetry(
	props: ToolResultOpts,
	promptContext: IBuildPromptContext,
	invokeOutcome: ToolInvocationOutcome,
	validateOutcome: ToolValidationOutcome,
	endpointProvider: IEndpointProvider,
	telemetryService: ITelemetryService,
) {
	const model =
		promptContext.request?.model &&
		(await endpointProvider.getChatEndpoint(promptContext.request?.model))
			.model;
...
class ToolResultElement extends PromptElement<
	IToolResultElementActualProps & BasePromptElementProps,
	void
> {
	async render(state: void, sizing: PromptSizing) {
		const { extraMetadata, toolResult, isCancelled } =
			await this.props.call(sizing);
		const toolResultElement = this.props.enableCacheBreakpoints ? (
			<>
				<Chunk>
					<ToolResult
						content={toolResult.content}
						truncate={this.props.truncateAt}
						toolCallId={this.props.toolCall.id}
						sessionId={this.props.sessionId}
						toolName={this.props.toolCall.name}
					/>
				</Chunk>
			</>
		) : (
		...
		return (
			<ToolMessage toolCallId={this.props.toolCall.id!}>
				...
				{toolResultElement}
				{this.props.isLast && this.props.enableCacheBreakpoints && (
	...
	new BasePromptRenderer(
		...
		class extends PromptElement {
			render() {
				return (
					<UserMessage>
						<PrimitiveToolResult content={toolResult.content} />
					</UserMessage>
				);
			}
		},
		...
		*/
			telemetry.sendMSFTTelemetryEvent(
				'agent.tool.responseLength',
				{ toolName },
				{ tokenCount },
			);
...
export async function imageDataPartToTSX(
	...
	if (isImageDataPart(part)) {
		...
		if (imageService) {
			try {
				...
				logService?.warn(
					`Image resize failed, using original: ${error}`,
				);
			}
		}
		...
		const isChatRequest =
			typeof urlOrRequestMetadata !== 'string' &&
		...
		if (githubToken && isChatRequest && imageService) {
			try {
				const uri = await imageService.uploadChatImageAttachment(
					imageData,
					'tool-result-image',
					mimeType ?? 'image/png',
					githubToken,
...
function toolResultToText(result: LanguageModelToolResult2): string {
	return result.content
		.filter(
			(part): part is LanguageModelTextPart | LanguageModelTextPart2 =>
				part instanceof LanguageModelTextPart ||
				part instanceof LanguageModelTextPart2,
		)
		.map((part) => part.value)
		.join('\n');
}
...
export function toolCallErrorToResult(err: unknown) {
	if (isCancellationError(err)) {
		return {
			result: textToolResult('The user cancelled the tool call.'),
			isCancelled: true,
		};
	} else {
		...
		return {
			result: textToolResult(
				`ERROR while calling tool: ${errorMessage}${toolErrorSuffix}`,
			),
		};
	}
}
...
class McpLinkedResourceToolResult extends PromptElement<
	{
		resourceUri: URI;
...
> {
	...
	async render() {
		...
		if (
			this.props.count >
			DONT_INCLUDE_RESOURCE_CONTENT_IF_TOOL_HAS_MORE_THAN
		) {
			return (
				<Tag
					name="resource"
					attrs={{ uri: this.props.resourceUri.toString() }}
				/>
			);
		}
		...
		try {
			contents = await this.fileSystemService.readFile(
			...
			const isNotFound =
				e instanceof Error &&
				'code' in e &&
				(e.code === 'FileNotFound' || e.code === 'EntryNotFound');
		...
		return (
			<>
				<Tag
					name="resource"
					attrs={{
						uri: this.props.resourceUri.toString(),
...
class PrimitiveToolResult<
	T extends IPrimitiveToolResultProps,
> extends PromptElement<T> {
	...
	constructor(
		...
		super(props);
		this.linkedResources = this.props.content.filter(
			(c): c is LanguageModelDataPart =>
				c instanceof LanguageModelDataPart &&
				c.mimeType === McpLinkedResourceToolResult.mimeType,
		);
	}
		...
		return (
			<>
				<IfEmpty alt="(empty)">
					{await Promise.all(
						this.props.content
							.filter((part) => this.hasAssistantAudience(part))
							.map(async (part) => {
								if (part instanceof LanguageModelTextPart) {
									...
									return await this.onTSX(
										part.value as JSONTree.PromptElementJSON,
									);
								} else if (isImageDataPart(part)) {
									return await this.onImage(
										part,
										this.props.content.indexOf(part),
									);
								} else if (
	...
	private hasAssistantAudience(
		...
		if (
			!(
				part instanceof LanguageModelDataPart2 ||
				part instanceof LanguageModelTextPart2
			) ||
			!part.audience
		) {
	...
	protected async onImage(part: LanguageModelDataPart, _imageIndex?: number) {
		...
		const githubToken = (
			await this.authService.getGitHubSession('any', { silent: true })
		)?.accessToken;
		...
		const uploadToken =
			uploadsEnabled && modelCanUseMcpResultImageURL(this.endpoint)
				? githubToken
				: undefined;
		...
		return Promise.resolve(
			imageDataPartToTSX(
				part,
				uploadToken,
				this.endpoint.urlOrRequestMetadata,
				this.logService,
				this.imageService,
			),
		);
...
export class ToolResult extends PrimitiveToolResult<IToolResultProps> {
	constructor(
		...
		private readonly diskSessionResources: IChatDiskSessionResources,
	) {
		super(
			props,
			endpoint,
	...
	protected override async onTSX(
		part: JSONTree.PromptElementJSON,
	): Promise<any> {
		if (this.props.truncate) {
			return (
				<TokenLimit max={this.props.truncate}>
					{await super.onTSX(part)}
				</TokenLimit>
			);
		}
	...
	protected override async onImage(
		part: LanguageModelDataPart,
		imageIndex?: number,
				...
				: part.mimeType === 'image/jpeg'
					...
					: part.mimeType === 'image/gif'
						...
						: part.mimeType === 'image/webp'
							? '.webp'
							: '.bin';
		const uri = buildToolImageResourceUri(
			this.props.sessionId,
			coreToolCallId,
			imageIndex,
			ext,
	...
	protected override async onText(content: string): Promise<string> {
		...
		if (
			isDiskCachingEnabled &&
			this.diskSessionResources &&
			this.props.toolCallId &&
			this.props.sessionId &&
			this.props.toolName !== ToolName.SearchSubagent &&
			this.props.toolName !== ToolName.ExecutionSubagent &&
			this.props.toolName !== ToolName.Memory
		) {
			...
			if (content.length > thresholdBytes) {
				try {
					...
					const schemaFileUri = schema
						? URI.joinPath(fileUri, 'schema.json')
						: undefined;
					this._logService?.debug(
						`[ToolResult] Large tool result (${content.length} bytes) written to disk: ${filePath}`,
					);
					...
					this._logService?.warn(
						`[ToolResult] Failed to write large tool result to disk: ${toErrorMessage(err)}`,
					);
		...
		if (!truncateAtTokens || content.length < truncateAtTokens) {
			// always >=1 character per token, early bail-out
			return content;
		}

		const tokens = await this.endpoint
			.acquireTokenizer()
			.tokenLength(content);
		...
		const targetChars = Math.round(
			approxCharsPerToken * (truncateAtTokens - removedMessage.length),
		);
		...
		return (
			content.slice(0, keepInFirstHalf) +
			removedMessage +
			content.slice(-keepInSecondHalf)
		);
	}
	...
	protected override onResourceLink(data: string) {
		...
		return (
			<McpLinkedResourceToolResult
				resourceUri={URI.revive(parsed.uri)}
				mimeType={parsed.underlyingMimeType}
				count={this.linkedResources.length}
			/>
		);
	}
}
...
export class ToolCallResultWrapper extends PromptElement<IToolCallResultWrapperProps> {
	async render(): Promise<PromptPiece | undefined> {
		return (
			<>
				{Object.entries(this.props.toolCallResults ?? {}).map(
...
export function buildToolImageResourceUri(
	sessionId: string,
	coreToolCallId: string,
	imageIndex: number,
	ext: string,
): string {
